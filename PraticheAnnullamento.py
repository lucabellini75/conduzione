...
def elaboraPratiche(pratiche,stato,inc,contDocUsc,contDoc,contSoloPcid,contPra):  
...
# print etichetta.etichettaPraticaIntesta()
    ...
    for pratica in pratiche:
        ...
        for i in pratica:
            ...
            if not isEmpty:
                ...
                for rows in cursore:    
                    if str(rows[8]) == "6" or str(rows[8]) == "4" or str(rows[8]) == "3" or str(rows[8]) == "2":
                        if str(rows[8]) == "2" or str(rows[8]) == "4":             
                            ...
                            #queryPraFas = queryCercaPratica.pratiche(rows[0])
                            #cursorePratiche = lanciaQueryAnnulla(queryPraFas)
                ...
                queryPraFas = queryCercaPratica.pratiche(pratica[0])
                cursorePratiche = lanciaQueryAnnulla(queryPraFas)
                ...
                queryCp = queryCercaPratica.annullaQSNSI(i)
                cursore = lanciaQueryAnnulla(queryCp)
    ...
    print etichetta.etichettaBancaDatiPraFas(inc, contPra)
    if contSoloPcid > 0:
    ...
    for item in dataCP:
        print "Processo da eliminare su Scrivania --> "+str(item)
...
if __name__ == '__main__':
    class mainexecution:
        ...
        elaboraPratiche(pratiche,stato,inc,contDocUsc,contDoc,contSoloPcid,contPra)
